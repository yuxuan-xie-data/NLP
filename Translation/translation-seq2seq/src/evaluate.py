...
def run_evaluate():
    ...
    model = TranslationModel(zh_tokenizer.vocab_size, en_tokenizer.vocab_size, zh_tokenizer.pad_token_index,
                             en_tokenizer.pad_token_index, config.ENCODER_NUM_LAYERS, config.DECODER_NUM_LAYERS).to(device)
    model.load_state_dict(torch.load(config.MODELS_DIR / 'best.pt', map_location=device)['model_state_dict'])
    print("模型加载成功")
