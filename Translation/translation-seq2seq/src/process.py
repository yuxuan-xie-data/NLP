...
def process():
    print("开始处理数据")
    # 读取文件
    df1 = pd.read_csv(config.RAW_DATA_DIR / 'cmn.txt', sep='\t', header=None, usecols=[0, 1], names=['en', 'zh'],
                     encoding='utf-8').dropna()
    df2 = pd.read_csv(config.RAW_DATA_DIR / 'bigger_data.csv', encoding='utf-8-sig').dropna()
